...
class KnopkaAmal(QPushButton):
    def __init__(
        ...
        if eni != None:
            self.setFixedWidth(eni)
        if buyi != None:
            self.setFixedHeight(buyi)
...
class Kalkulyatr(QMainWindow):
    ...
    def bajar(self, k: str) -> None:
        ...
        for i in range(len(yzv) - 1, 0, -1):
            if yzv[i].isdigit() and yzv[i - 1].isdigit():
                n += 1
            else:
                n = 0
            if n == 3:
    ...
    def bajar2(self, k: str) -> None:
        self.yuqot()
        yzv = self.ntj.text()
        if yzv == "0" and (k.isdigit() or k in self.blg):
            yzv = ""
        self.bajar(yzv + k)
    ...
    def natija(self) -> None:
        ...
        try:
            yzv = self.ntj.text()
            if "π" in yzv:
                yzv = yzv.replace("π", "pi")
            if "^" in yzv:
                yzv = yzv.replace("^", "**")
            if "√" in yzv:
                yzv = yzv.replace("√", "sqrt")
    ...
    def start(self) -> None:
        ...
        for i in self.num9_1:
            stl = i.text()
            if stl == "=":
                i.bosibTurish(2, self.funkely, self.natija)
            elif stl == "del":
                i.bosibTurish(1, self.uchir2, self.uchir)
            elif stl == "close":
                i.quyibYuborish(self.close)
            else:
                if stl == "√":
                    stl = "√("
