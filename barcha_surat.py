from PyQt5.QtWidgets import (
    QApplication, QLabel, QLineEdit, QMainWindow, QPushButton, QWidget)
...
class Filelar(object):
    ...
    def start(self, path:str, files:list=["jpg","png"], all:bool=False) -> list:
        ...
        for i in listdir(path):
            pth = path +'/'+ i
            if not isfile(pth):
                try: self.start(pth, files, all)
                except: pass
            else:
                if all: self.__path.append(pth)
                else:
                    for j in files:
                        if i.endswith('.'+j):
...
class Window(QMainWindow):
    def __init__(self) -> None:
        ...
        self.__files = [
            "jpg", "png", "jpeg", "gif", "tiff", "psd",
            "pdf", "eps", "ai", "indd", "raw", "jfif", "tif"]
        self.__oby = Filelar()
    
    def bajar(
        self, obj:QWidget, x:int, y:int, mashtab:int=10,
        sozlash:bool=False, rang:str=None, eni:int=None, buyi:int=None
    ) -> None:
        if mashtab is not None:
            obj.setFont(QFont("Times", mashtab))
        if not sozlash: obj.move(x, y)
        else:
            obj.move(x-obj.fontMetrics().boundingRect(obj.text()).width(), y)
        if rang is not None:
            obj.setStyleSheet(f"background: {rang}")
        obj.adjustSize()
        if eni is not None: obj.setFixedWidth(eni)
        if buyi is not None: obj.setFixedHeight(buyi)
        ...
        if ad_ln > 0:
            while True:
                ...
                try:
                    ...
                    self.__yz[3].setText(f"{n+1} - surat\n(q harfini bossangiz chiqasiz)")
